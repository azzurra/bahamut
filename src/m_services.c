...
int m_os(aClient *cptr, aClient *sptr, int parc, char *parv[]) 
{
    ...
    if ((acptr = find_server(OPERSERV_SERVER, NULL)))
#ifdef SERVICESHUB
	sendto_one(acptr, ":%s OS :%s", parv[0], parv[1]);
#else
	sendto_one(acptr, ":%s PRIVMSG %s@%s :%s", 
		parv[0], OPERSERV, OPERSERV_SERVER, parv[1]);
#endif
    else
	sendto_one(sptr, err_str(ERR_SERVICESDOWN), me.name,
		   parv[0], OPERSERV);
...
#else
	sendto_one(acptr, ":%s PRIVMSG %s@%s :%s", 
		parv[0], SS_SERVICE, STATS_NAME, parv[1]);
#endif
    else
	sendto_one(sptr, err_str(ERR_SERVICESDOWN), me.name,
		   parv[0], SS_SERVICE);
...
int m_st(aClient *cptr, aClient *sptr, int parc, char *parv[]) 
{
    ...
    if ((acptr = find_server(STATS_NAME, NULL)))
#ifdef SERVICESHUB
	sendto_one(acptr, ":%s ST :%s", parv[0], parv[1]);
#else
	sendto_one(acptr, ":%s PRIVMSG %s@%s :%s", 
...
int m_hs(aClient *cptr, aClient *sptr, int parc, char *parv[]) 
{
    ...
    if ((acptr = find_server(HELPSERV_SERVER, NULL)))
#ifdef SERVICESHUB
	sendto_one(acptr, ":%s HS :%s", parv[0], parv[1]);
#else
	sendto_one(acptr, ":%s PRIVMSG %s@%s :%s", 
		parv[0], HELPSERV, HELPSERV_SERVER, parv[1]);
#endif
    else
	sendto_one(sptr, err_str(ERR_SERVICESDOWN), me.name,
		   parv[0], HELPSERV);
...
#define HAS_FLIPPED(x)                                  \
	...
	case 'o':
	    if (MyClient(acptr))
	    {
		if (what == MODE_ADD && !IsOper(acptr))
		{
		    addto_fdlist(acptr->fd, &oper_fdlist);
		    /* Assign standard oflags as well */
		    acptr->oflag = OFLAG_GLOBAL | OFLAG_SPAM | OFLAG_DIE | OFLAG_RESTART | OFLAG_GSHOWIP;
		    acptr->umode |= UMODE_o;
		    Count.oper++;
		}
		else if (what == MODE_DEL && IsOper(acptr))
		{
	...
	case 'O':
	    /* Don't mess around with global ops... */
	    if (MyClient(acptr) && !IsOper(acptr))
	    {
		if (what == MODE_ADD && !IsLocOp(acptr))
		{
		    addto_fdlist(acptr->fd, &oper_fdlist);
		    /* Local oflags */
		    acptr->oflag = OFLAG_LOCAL | OFLAG_LSHOWIP;
		    acptr->umode |= UMODE_O;
		    Count.oper++;
		}
		else if (what == MODE_DEL && IsLocOp(acptr))
		{
		    delfrom_fdlist(acptr->fd, &oper_fdlist);
		    /* reset client confs */
		    det_confs_butmask(acptr, CONF_CLIENT & ~CONF_OPS);
	    ...
	    /* Not our client - ignore the change */
	    break;
