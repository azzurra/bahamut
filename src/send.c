...
int send_queued(aClient *to)
{
    ...
    size_t len2; /* get rid of strict-aliasing rules... */
    ...
    {
	msg = dbuf_map(&to->sendQ, &len2); /*AZZURRA*/
	len = len2;
				...
				char *buffer, char *pattern, va_list vl)
{
    ...
    *buffer = ':';
	...
	if (user)
	{
	    ...
	    if (*host && !MyConnect(from)) /*AZZURRA*/
	    {
		buffer[sidx++] = '@';
		for(p = host; *p; p++) /*AZZURRA*/
		    buffer[sidx++] = *p;
		flag = 1;
	    }
	}
 ...
 */
void sendto_security(char *chname, char *pattern, ...) 
{
    aChannel *chptr;
    chanMember *cm;
    aClient *acptr;
    ...
    if (!chname)
       chname = SECURITY_CHANNEL; 
    if (!(chptr=find_channel(chname, NullChn)))
        return;
	
    ircsprintf(nbuf, ":%s PRIVMSG %s :", me.name, chptr->chname);
...
void send_quit_to_common_channels(aClient *from, char *reason)
{
	...
	for (users = channels->value.chptr->members; users; users = users->next)
	{
	    ...
	    /* Stop sending quits to non opers when the channel has
	     * MODE_NOSPAM and when the user is not allowed to
	     * send text to the channel. -INT */
	    if (!IsAnOper(cptr) && (!(from->flags & FLAGS_KILLED)) && 
		(!(reason[0] == 'L')) &&
		((channels->value.chptr->mode.mode & MODE_NOSPAM) ||
		 (can_send(from, channels->value.chptr, reason))))
	        continue;
...
void send_part_to_common_channels(aClient *from, char *reason)
{
    ...
    for (channels = from->user->channel; channels; channels = channels->next)
    {
	if (((channels->value.chptr->mode.mode & MODE_NOSPAM) ||
	     can_send(from, channels->value.chptr, reason)) &&
	     (!(from->flags & FLAGS_KILLED)) && (!(reason[0] == 'L')))
	{
		...
		if (!MyConnect(cptr) || sentalong[cptr->fd] == sent_serial)
	        ...
	        /* We have already sent quit message to opers -INT */
	        if (IsAnOper(cptr))
		    continue;
...
void sendto_snotice(char *pattern, ...)
{
	...
	if ((cptr = local[i]) && !IsServer(cptr) && !IsMe(cptr) &&
	    ...
	    (void) ircsprintf(nbuf, ":%s NOTICE %s :*** Spam -- ",
			      me.name, cptr->name);
	    (void) strncat(nbuf, pattern,
			   sizeof(nbuf) - strlen(nbuf));
	    vsendto_one(cptr, nbuf, vl);
 ...
 *   Send a message to all flagged users in channel chptr that
 *   are directly on this server and sends the message
 *   on to the next server if it has any flagged users.
 */
void sendto_channelflag_butone(aClient *one, aClient *from, int typedest,
				aChannel *chptr, char *pattern, ...) 
{
	...
	/* Skip the specified one and users that do not have the
	 * correct flags */
	if (acptr->from == one ||
...
void dump_connections(int fd) 
{
	...
	if (fd >= 0 && (cptr = local[fd]) && DBufLength(&cptr->sendQ) > 0)
	    send_queued(cptr);
}
