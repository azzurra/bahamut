...
void report_conf_links(aClient *sptr, aConfList *List, int numeric, char c)
{
    aConfItem *tmp;
    int current;
    char       *host, *pass, *name;
...
int sortable(char *my_string)
{
	...
	case 1:
	    if (!*p)		/* only wildcards, not good -Sol */
		return (0);
	    else if (*p != '*')
		state = 2;
	    break;
	...
	case 4:
		...
		/* what sol's orig returns, but 1 will be
		 * better, since it wont have to reverse 
		 */
		return (1);
	    break;
...
char *host_field(aConfItem *my_conf)
{
    char       *tmp;
    char       *ptr;
...
char *name_field(aConfItem *my_conf)
{
    char       *tmp;
    char       *ptr;
...
char *rev_host_field(aConfItem *my_conf)
{
    char       *tmp;
    char       *ptr;
...
char *rev_name_field(aConfItem *my_conf)
{
    char       *tmp;
    char       *ptr;
 ...
 * reallocated. -Sol
 */
static int grow_list(aConfList *my_list)
{
...
void addto_conf_list(aConfList *my_list, aConfItem *my_conf, 
		     char *(*cmp_field)())
{
...
void clear_conf_list(aConfList *my_list)
{
    if (!my_list)
	return;
 ...
 */
aConfItem *find_matching_conf(aConfList *my_list, char *tomatch)
{
 ...
 * -Sol
 */
void l_addto_conf_list(aConfList *my_list, aConfItem *my_conf,
		  char *(*cmp_field)())
{
    char       *field = cmp_field(my_conf);
    unsigned int length = my_list->length;
 ...
 * chained entries. -Sol
 */
aConfItem *l_find_matching_conf(aConfList *my_list, char *tomatch)
{
	...
	if (!base[current].pattern)
	    return base[current].conf;	/* Let's say NULL matches all -Sol */
